...
function LoadInhouseReservationGrid() {
    //alert("2");
    SetNoOfRowsInGrid();
    ...
    $.ajax({
        url : "/reservation/InhouseReservationList/InhouseReservations",
        dataType: 'json',
        method: 'POST',
        data: pagination,
        beforeSend: function () {
...
function DrawReservationRow(item, tableContainerId, appendOrAfter) {
    ...
    if (item.StopPosting) {
        IsStopPosting = '<input type="hidden" value="1" class="isStopPosting" /><text>Stopped</text>';
    } else {
        IsStopPosting = '<input type="hidden" value="0" class="isStopPosting" /> <text>Allowed</text>';
    }
    ...
    var dataRow = $(
        ...
        '<td>' +
        '<div style=" margin-left:-30px;" tabindex="0"   ' +
        'role="button"' +
        'data-html="true"' +
        'data-toggle="popover"' +
        'data-trigger="hover"' +
...
$(document).off('click', '.data-reservation-no').on('click', '.data-reservation-no', function () {
    var resHeaderId = $(this).attr('reservation-header-id')

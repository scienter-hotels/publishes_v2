...
function LoadReservationGrid() {
    ...
    $.ajax({
        url: "/reservation/ReservationList/ReservationList",
        dataType: 'json',
        method: 'POST',
        data: pagination,
        beforeSend: function () {
...
function DrawReservationRow(item, tableContainerId, appendOrAfter) {
    ...
    //if (item.GroupColour != 'Single') {
    //    groupHighlight = 'style="border-bottom:3px solid ' + item.GroupColour + ' !important; border-top:3px solid ' + item.GroupColour + ' !important;"';
    //}

    //if (item.NoOfAdults > 0) {
    //    noOfAdults = '<span class="glyphicon glyphicon-user" style="color:black; font-size:15px;" data-toggle="tooltip"></span><sup class="superscript">' + item.NoOfAdults + '</sup>'
    //} else {
    //    noOfAdults = '<span class="glyphicon glyphicon-user" style="color:#808080; font-size:15px;" data-toggle="tooltip"></span><sup class="superscript">0</sup>';
    //}

    //if (item.NoOfKids > 0) {
    //    noOfKids = '<span class="fa fa-child " style="color:black; font-size:15px;" data-toggle="tooltip"></span><sup class="superscript">' + item.NoOfKids + '</sup>'
    //} else {
    //    noOfKids = '<span class="fa fa-child " style="color:black; font-size:15px;" data-toggle="tooltip"></span><sup class="superscript">0</sup>'

    //}

    //if (item.HasAlert == 1) {
    //    hasAlert = '<span class="glyphicon glyphicon-bell " style="color:#555; font-size:15px;" data-toggle="tooltip" title="Alert"></span>';
    //} else {
    //    hasAlert = '<span class="glyphicon glyphicon-bell" style="color:#F2F2F2; font-size:15px;" data-toggle="tooltip" title="Alert"></span>'
    //}

    //if (item.HasTravel == 1) {
    //    HasTravel = '<span class="fa fa-taxi" style="color:#555; font-size:15px;" data-toggle="tooltip" title="Transport"></span>';
    //} else {
    //    HasTravel = '<span class="fa fa-taxi" style="color:#F2F2F2; font-size:15px;" data-toggle="tooltip" title="Transport"></span>';
    //}

    //if (item.HasTraces == 1) {
    //    HasTraces = '<span class="fa fa-check" style="color:#555; font-size:15px;" data-toggle="tooltip" title="Traces"></span>';
    //} else {
    //    HasTraces = '<span class="fa fa-check" style="color:#F2F2F2; font-size:15px;" data-toggle="tooltip" title="Traces"></span>';
    //}
    ...
    var allotmentImage = "/images/not-attach-to-allotment.png";
    if (item.IsAttachToAllotment) {
        allotmentImage = "/images/attach-to-allotment.png";
    }

    // --- Vbutler / Self check-in status -----------------
    var vbutler = (item.VButlerLoginStatus || '').trim().toLowerCase();
    var vButlerImage = "/content/reservation-list-icons/vb-not-sent.png";
    var vButlerTitle = "Link not sent";

    if (item.IsVButlerLinkSent == 1) {
        if (vbutler === 'logged') {
            vButlerImage = "/content/reservation-list-icons/vb-logged.png";
            vButlerTitle = "Link sent • Guest logged in";
        } else {
            vButlerImage = "/content/reservation-list-icons/vb-sent.png";
            vButlerTitle = "Link sent • Not logged in yet";
        }
    } else {
        if (vbutler === '-' || vbutler === '') {
            vButlerImage = "/content/reservation-list-icons/vb-not-sent.png";
            vButlerTitle = "Link not sent";
        }
    }
// ----------------------------------------------------
    ...
    //Meal reservation display
    var selectionCheckbox = "";


    if (item.RoomCategoryPrefix != "MEAL") {
        selectionCheckbox = '<div class="form-group checkbox" style="margin-left: 5px;">' +
            '<label>' +
        ...
        selectionCheckbox = '<a class="btn btn-primary btn-theme-color-2 btn-sm" href="/mealreservation/mealreservation/landing"><span class="glyphicon glyphicon-share-alt"></span></a>';
    }
    ...
    var dataRow = $(
        ...
        '<td class=""  title="Allotment">' +
        '<img src="' + allotmentImage + '" style="width:20px;" title="Attach to allotment">' +
        '</td>' +

        // Vbutler column
        '<td class="" title="' + vButlerTitle + '">' +
        '<img src="' + vButlerImage + '" style="width:30px;" />' +
        '</td>' +
        ...
        item.GuestName +
        '</td>' +
        '<td class="vip-level" data-vipLevel="' + item.VipLevel + '">' +
        item.VipLevel +
        '</td>' +  
...
function ColomnHideOnSideButtonOpen() {
    if (isSideButtonsOpened) {
        $('#existingReservationGrid th.hide-on-side-buttons-new').each(function () {
...
$('body').on('click', '.group-reservation-view', function () {
    ...
    if (reservationCount > 1) {
        ...
        if ($(this).hasClass('open-group') == false) {
            ...
            $.ajax({
                url: "/reservation/ReservationList/GroupReservationsDetails",
                dataType: 'json',
                method: 'POST',
